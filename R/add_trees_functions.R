startdate<-"2020-01-31 GMT"
enddate<-"2023-12-31 GMT"
addtrees_hadukdata<-function(startdate, enddate, dtmc,
  ...
  # Create base url
  basepath<-file.path("D:/badc/ukmo-hadobs/data/insitu/MOHC/HadOBS/HadUK-Grid/v1.2.0.ceda/1km/")
  ...
  for (v in varns){
    ...
    e<-crop(extend(crop(r[[1]],dtmc,snap='out'),1),r[[1]])
    rout<-crop(r,e)
...
get_ukcp_dtm<-function(aoi){
  # Load dtm
  fname<-'orog_land-rcm_uk_12km_osgb.nc'
  path<-'D:/badc/ukcp18/data/land-rcm/ancil.orog'
  ...
  dtmc_ext<-crop(extend(crop(dtmc,aoiproj,snap='out'),1),dtmc)
  dtmc_out<-crop(dtmc,dtmc_ext)
  # plot(dtmc_out,main='UKCP18 climate area and bounding box of downscaling parcels')
  # plot(aoiproj,add=T)
  return(dtmc_out)
}

## ADD a get_dtmm function inputs: dtmc, dtmf
aoi<-terra::vect('D:/mesoclim_inputs/parcels')
plot(aoi)
startdate<-as.POSIXlt('2024-01-01')
enddate<-as.POSIXlt('2039-12-31')
...
climdata<-addtrees_climarray(aoi,startdate,enddate,collection='land-rcm',domain='uk',member='01')

sstdata<-addtrees_sstdata(startdate,enddate,aoi=climdata$dtm,member='01')
...
addtrees_climdata <- function(aoi,  startdate, enddate,
  ...
  # Jasmin basepath
  basepath<-file.path("D:/badc","ukcp18","data",collection,domain,collres,rcp)

  # Create coarse-resolution dtm to use as template for cropping etc !!!
  dtmc<-get_ukcp_dtm(aoi)
  units(dtmc)<-'m'
...
addtrees_sstdata<-function(
  ...
  filepath<-file.path('D:/badc/deposited2023/marine-nwsclim/NWSPPE',modelid,'annual')
